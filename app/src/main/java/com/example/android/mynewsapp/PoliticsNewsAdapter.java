...
public class PoliticsNewsAdapter extends RecyclerView.Adapter<PoliticsNewsAdapter.ViewHolder> {
    private ArrayList<News> mLatestNewsArray = new ArrayList<>();
    private final PoliticsNewsFragment.OnListFragmentInteractionListener mListener;
    static Context mContext;

    /**
     * This is a public constructor used to create an object of the  adapter class
     * {@link PoliticsNewsAdapter}
     */
    public PoliticsNewsAdapter(ArrayList<News> latestNewsArray,
                               PoliticsNewsFragment.OnListFragmentInteractionListener listener, Context context) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position, List<Object> payloads) {
        ...
        if (position % 2 == 0) {
            holder.politicsLayout.setBackgroundColor(color.rgb(251, 233, 231));
        } else {
            holder.politicsLayout.setBackgroundColor(color.rgb(255, 235, 238));
        }
        // Sets the color of the view that represents the section name of the news
        if (position == 1) {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.politics_news_list, parent, false);
